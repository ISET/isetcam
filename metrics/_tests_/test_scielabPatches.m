function tests = test_scielabPatches()
...
%
% See also:  
%    s_scielabMTF - shows the effect of spatial frequency on the
...
% Store a hash of the quantized dES values.  Weird.  Why not just use
% mean(dES(:))
assert( abs(mean(dES(:)) - 5.574721542812232) < 1e-2);
