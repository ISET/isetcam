function tracklet = smoothTrack(obj,tracklet)
%    lambda = [1.5 1.5 4 4].^2;
    faceRects = cat(2,tracklet(:).rect);
...
%    faceRectsSmoothed = reshape(M5\pts,4,[]);
    faceRectsSmoothed = obj.smoothRects(faceRects);   
    for i=1:numel(tracklet)
        tracklet(i).rectSmoothed = faceRectsSmoothed(:,i);
    end
