...
if checkfields(parms, 'ang'), ang = parms.ang;
else ang = 0;
    parms.ang = ang;
end
if checkfields(parms, 'contrast'), contrast = parms.contrast;
else contrast = 1;
    parms.contrast = contrast;
end
if checkfields(parms, 'freq'), freq = parms.freq;
else freq = 1;
    parms.freq = freq;
end
if checkfields(parms, 'ph'), ph = parms.ph;
else ph = pi / 2;
    parms.ph = ph;
end
if checkfields(parms, 'row'), row = parms.row;
else row = 64;
    parms.row = row;
end
if checkfields(parms, 'col'), col = parms.col;
else col = 64;
    parms.col = col;
end
