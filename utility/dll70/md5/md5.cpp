...
//
// The mex wrapper is copyright (c) 2004, Bob Dougherty. All rights
// reserved. 
//
...
/*********************************************************************
 * Compute md5 hash on an char array input.
 *
 * To compile, run:
 *    mex -O md5.cpp
 *
...
extern "C" void mexFunction(int nlhs, mxArray *plhs[],
  ...
  }else if (!mxIsChar(prhs[0]) | mxGetM(prhs[0])!=1){
    /* First arg must be a real 1xN array */
    mexErrMsgTxt("Argument must be a string.");
  }else {
    int msgLen = (mxGetM(prhs[0]) * mxGetN(prhs[0])) + 1;
    char *msgStr = (char *)mxCalloc(msgLen, sizeof(char));
    if(mxGetString(prhs[0], msgStr, msgLen))
      mexWarnMsgTxt("Not enough space- string is truncated.");
    plhs[0] = mxCreateString(MD5String(msgStr));
  }
}
