...
%%%%% COMMENTS
% The implementation can be easily modified to have G sample at the upper
% left corner. (Does not affect much...)
...
Rdu = R;
Rdu(1:height-1, 1:width-1) = Rd2; %figure; imshow(uint8(Rdu));
Bdu = B;
Bdu(2:height, 2:width) = Bd2; %figure; imshow(uint8(Bdu));
...
z = conv2(x, h0);
a = conv2(z, h0');
a = a(t0:t0+height-1, t0:t0+width-1);
h = conv2(z, h1');
h = h(t0:t0+height-1, t0:t0+width-1);

z = conv2(x, h1);
v = conv2(z, h0');
v = v(t0:t0+height-1, t0:t0+width-1);
d = conv2(z, h1');
d = d(t0:t0+height-1, t0:t0+width-1);
