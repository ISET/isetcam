function results = colorUnitTest(mode)
% COLORUNITTEST - Run color tests in the _tests_ directory.
%
% Usage:
%   results = colorUnitTest;
%   results = colorUnitTest('full');
%
...
coreTests = {'test_colorConversions/', ...
    'test_colorFundamentals/', ...
    'test_colorTransforms/'};

fullOnlyTests = {'test_chart/', ...
    'test_macbeth/'};
...
switch mode
    ...
    otherwise
        error('Unknown colorUnitTest mode %s. Use ''core'' or ''full''.',mode);
end

runner = TestRunner.withTextOutput;
results = runner.run(suite);
ieTestReport(results,'colorUnitTest');
