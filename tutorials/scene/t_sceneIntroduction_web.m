...
% *Description*
% 
% ISETCam is structured around a few key objects that are important parts of 
% the image acquisition pipeline. These are the *scene, optical image, sensor, 
% and image processor*. This script introduces the WsISETCam *scene*.
% 
% A main goal of this script is to illustrate the ISETCam programming style. 
% By using ISETCam objects properly, the code and analysis are much easier to 
% understand. 
% 
% The implementation of these objects was written before Matlab implemented 
% its classes and thus doesn't rely on that feature. But many of the principles 
% are the same. For each object there are three fundamental operations: *Create, 
% set parameters, and get* object parameters.
% 
% The scene describes the spectral radiance field. For the present tutorial 
% we work with a simple planar radiance image, such as the image on a display 
% surface.
% 
% We have substantially extended ISETCam for implementations for 3D scenes in 
% the github repository (iset3d).
% 
% *Note*
%% 
...
%% 
% When the window appears, you can scale the window size and adjust the font 
...
% sample spacing.
% 
% In designing the sets/gets, the author must select an approach:  Do we allow 
% the user to set anything and then update the other parameters for consistency, 
% or do we only allow the user to set specific parameters and through this limitation 
% enforce consistency.  ISETCam uses the 2nd method: Only some parameters can 
% be set.
% 
...
% you are ever tempted to set the objects directly, go get a cup of coffee. When 
...
%% 
% If we move the scene closer, and we maintain the same number of row and column 
% samples, the spacing changes.
...
sceneGet(scene,'sample spacing','mm')
%% Specifying units
% In this example, we specified the units of the sample spacing as millimemters 
% (mm). We could have specified microns.
...
% ISETCam includes a few multispectral scenes.  These are large files, so the 
% default distribution only includes one example.  We have another 100 or so that 
% are available.
