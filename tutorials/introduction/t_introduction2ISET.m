...
% An overview of *ISETCam* illustrating how to perform basic image system simulation.
% 
% There are a number of Matlab *live scripts* in the tutorials folder that describe 
% more about how to interact with the components, such as scenes, optical images, 
% sensors and image processing.  There are also a large number of m-file scripts 
% that are examples of calculations pertaining to scenes, optics and sensors.
% 
% To find these scripts and tutorials use the tab-completion function. For example
%% 
...
%% 
% To learn more about the processing for a particular function, type using Matlab's 
% doc command, such as *doc sceneCreate.*
% 
% See also: 
% 
% *t_sceneIntroduction.mlx* and *t_oiIntroduction.mlx*
%% Initialize ISET variables and structures
% The ieInit function is often used at the beginning of a calculation to clear 
% up the workspace.  The function closes windows, and it can be set to clear your 
% variables (depending on how you set your ISET preferences).
...
% 
% A second method for creating a scene is to read data from a file. ISET includes 
% a few multispectral in the _data/image/multispectral_ directory 
% 
% *See also:* 
%% 
% * s_sceneFromMultispectral.m
% * s_sceneFromRGB.m 
% * scripts/scenes/*
...
scenePlot(scene,'illuminant energy roi');
%% 
% You can adjust properties using the sceneSet command. Here we adjust the scene 
% mean luminance and field of view.
...
% 
% ISET has two optics models that you can experiment with. These are shift-invariant 
% optics, in which there is a different shift-invariant pointspread function for 
% each wavelength, and a ray-trace method, in which we read in data from Zemax 
% and create a shift-variant set of pointspread functions along with a geometric 
% distortion function.  The most common method is a shift-invariant calculation 
% in which we specify the optics using its wavefront aberration. 

oi = oiCreate('wvf');
%% 
% This default creates an optical image is to uses a diffraction-limited lens 
% model.  To create diffraction limited optics with an f# of 12, which will blur 
% the image substantially, you can call these functions

oi = oiSet(oi,'optics fnumber',2.8);

%% 
% In this example we set the properties of the optics to include cos4th falloff 
% for the off axis vignetting of the imaging lens and we set the lens focal length 
% to 3 mm.
...
% Next, we view the optical image window.
oiWindow(oi);
%% 
% Many other optics and oi properties can be set. For a list see *doc opticsSet, 
% and doc oiSet.*  The value of using the wavefront to describe the optics is 
% that we can set the parameters of the wavefront aberration to produce many different, 
% physically plausible, point spread functions. To see the relationship between 
% the wavefront parameters and the point spread function, see the tutorials and 
% scripts that with names t_wvf* and s_wvf*.
% 
% From the OI window you can see a wide range of options to both set and measure. 
% These include insertion of a birefringent anti-aliasing filter, turning off 
% cos4th image fall-off, adjusting the lens properties, and so forth.  For these 
% general properties, look at the tutorials and scripts t_oi* and s_oi*.
% 
% You can read more about optics models properties that we compute for you by 
% typing  *doc opticsGet*.
%% Sensor
% The irradiance is then captured by a simulated sensor, resulting in an array 
% of output voltages. There are a very large number of sensor parameters. Here 
% we illustrate the process of creating a simple Bayer-gbrg sensor and setting 
% a few of its basic properties.

% To create the sensor structure, we call
sensor = sensorCreate('bayer (rggb)');
%% 
% We set the sensor properties using *sensorSet* and *sensorGet* routines. Just 
% as the optical irradiance gives a special status to the optics, the sensor gives 
% a special status to the pixel.  In this section we define the key pixel and 
% sensor properties, and we then put the sensor and pixel back together.
...
%% 
% It is also possible to replace the spectral quantum efficiency curves of the 
% sensor with those from a calibrated camera.  We include the calibration data 
% from a very nice Nikon D100 camera as part of ISET. 
% 
...
%% 
% We can view sensor image in the GUI.  Note that the image that  comes up shows 
% the color of each pixel in the sensor mosaic. Also, please be aware that the 
% Matlab rendering algorithm often introduces unwanted artifacts into the display 
% window.  You can resize the window to eliminate these. You can also set the 
% display gamma function to brighten the appearance in the edit box at the lower 
% left of the window.
...
% Also, you can view the individual pixel data either by zooming on the image 
% (Edit | Zoom) or by bringing the image viewer tool (Edit | Viewer).
% 
% ISET includes a wide array of options for selecting color filters, fill-factors, 
% infrared blocking filters, adjusting pixel properties, color filter array patterns, 
% and exposure modes.
%% Image Processor
% The sensor array is demosaiced, color-balanced, and rendered on a display 
% The image processing pipeline is managed by the fourth principal ISET structure, 
% the virtual camera image (ip). This structure allows the user to set a variety 
% of image processing methods, including demosaicking and color balancing.
...
% 
% 
% Again, this window offers the opportunity to perform many parameter changes 
% and to evaluate certain metric properties of the current system. Try the pulldown 
% menu item (Analyze | Create Slanted Bar) and  then run the pulldown menu (Analyze 
% | ISO12233) to obtain a spatial frequency response function for the slanted 
% bar image in the ISO standard.
