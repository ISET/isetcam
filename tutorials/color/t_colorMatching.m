...
%% 
% This tutorial was initially used to teach Psych 221.  More detailed tutorials 
% are available in the psych221 repository on GitHub.

ieInit;
%% Computing XYZ values of a monitor
% We must measure the main functions that define the color properties of any 
% display: these are the phosphor spectral power distributions (SPDs). We measure 
% these functions using a spectral radiometer, a device that measures the power 
% per nanometer emitted by the display.
% 
% The spectral power distributions of a monitor in my lab, measured when the 
% monitor display intensity was set to full value, are stored in columns of the 
% matrix called "phosphors". This matrix is stored in the data file below
...
%% 
% To measure the XYZ values of these phosphors, we load in the xbar,ybar, zbar 
% functions defined by the CIE.  These are also stored in the columns of a matrix, 
% called xyz.  Because these functions are so widely used, they are stored in 
% the main toolbox area.
...
% phosphor matrix times the xyz matrix. Notice that the phosphors are in the columns 
% of the matrix on the right, and the xbar,ybar,zbar functions are in the rows 
% of the matrix on the left.

maxXYZ = XYZ'*phosphors

%% 
% This calculation doesn't take into account the proper constants and details 
% of the formula.  There is a routine that manages that for you.
...
%% 
% The total luminance the display can reach, with all three phosphors on, is 
% given by the sum of these three values, 100 cd/m^2 is a typical luminance level 
% for monitors.

sum(maxXYZ(2,:))
%% Using the CMFs to Match a target
% Suppose that you send a space ship to the surface of Mars. The spaceship sends 
% back a measurement of a spectral power distribution.  Let's load it up. This 
% time we suppose the display is only calibrated from 400 to 700 in 5 nm steps.
...
%% 
% Now, recall that the monitor color properties are determined by the relative 
% intensity of the outputs of the monitor phosphor SPDs.  Suppose these  intensities 
% are [r,g,b]'.  Then the monitor output is phosphors*[r,g,b]'.  For example, 
% when the red phosphor only is on the spectrum is
...
% only difference, which is reviewed below, is that the frame-buffer entries and 
% the intensity of the phosphor output are a nonlinearly related. Hence, we must 
% set the frame-buffer entries so that we obtain the linear intensity outputs 
% we have just calculated.  This process is called gamma correction.
%% Managing a nonlinear display - Gamma tables and inverse gamma tables
% The relationship between the intensity emitted by a CRT phosphor and the frame-buffer 
% value is generally a nonlinear function. An example of the function relating 
% frame-buffer value to emitted light intensity is shown here:
...
%% 
% This is called the "gamma" function of the display.  The reason for this title 
% is that the function is roughly a simple power function and the exponent has 
% historically been called "gamma." Here is a comparison of the gamma function 
% of the red phosphor and a power function with an exponent of 2.2, the most common 
% value.
...
% signals emitted when the relative intensity is one.
% 
% To specify the intensity of the emitted light for any given frame-buffer level,  
% we can use the simple gamma function.  For example, the spectral power distribution 
% of the light emitted by the green phosphor at a frame buffer level of 130 is
...
% the function.  So that given a linear intensity, l, we can calculate the frame-buffer 
% value as
% 
...
% inverts the gamma function.  You should take a look at the code in the function 
% "mkInvGammaTable" to see how Xuemei Zhang and I create inverse gamma look-up 
...
%% 
% Now, we plot these values and turn on hold.  Any light is a mixture of spectral 
% lights, so these values define an outer boundary for where any physical light 
% can fall.
...
% 
% b) Plot the SPD of the phosphors that would make this match. Why are these 
% intensities not physically realizable?
% 
% c) If you wanted to perform an experiment to test the predicted match what 
% could you do to arrange the viewing conditions?
% 
...
%{
 export('t_colorMatching.mlx', 't_colorMatching.html', 'Format','html');
%}