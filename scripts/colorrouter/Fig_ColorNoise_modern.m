...
sensorRGB = sensorCreate('custom', pixel, mosaicPattern, filterFile);

% Set physical size and array dimensions
sensorRGB = sensorSet(sensorRGB, 'pixel size constant fill factor', pixelSize_um * 1e-6); % µm->m
...
%---------------------------
irFilter   = sensorGet(sensorRGB, 'irfilter');   % IR-cut filter spectrum
spectralQE = sensorGet(sensorRGB, 'spectral qe');% combined spectral QE (per channel)
wave_sensor = sensorGet(sensorRGB, 'wave');      % wavelengths used by sensor
...
%---------------------------
sensorRGB = sensorSet(sensorRGB, 'pixel spectral qe', cmosQE); % intrinsic pixel QE
sensorRGB = sensorSet(sensorRGB, 'filter spectra', cf);       % color filter transmittances
...
sensorRGB = sensorSet(sensorRGB, 'noiseflag', -2);   % set noise model (example)
sensorRGB = sensorCompute(sensorRGB, oi);            % compute once to get exposure time
expTime = sensorGet(sensorRGB, 'exp time');         % read computed exposure time
...
% sensorRGB must exist in the workspace.
line1 = sensorGet(sensorRGB, 'hline electrons', rows_rgb(1));
line2 = sensorGet(sensorRGB, 'hline electrons', rows_rgb(2));
...
% Apply single-layer color router spectra to a copy of sensorRGB and visualize.
% Assumes `wave`, `sensorRGB`, and an optical image `oi` exist in the workspace.

% Read router optical-efficiency (OE) spectrum sampled at `wave`
router_oe = ieReadSpectra('singleLayerColorRouter', wave);

% Plot the router efficiency vs wavelength
ieFigure;
plot(wave, router_oe, 'LineWidth', 1.2);
grid on;
xlabel('Wavelength (nm)');
ylabel('Efficiency');
title('Single-Layer Color Router OE');

% Create a sensor copy and set the router spectra as its filter responses
sensor_router = sensorRGB;                         % copy base sensor
...
pSize = sensorGet(sensorRGB, 'pixel size');       % pixel size in meters
...
sonyCF = sensorGet(sensorRGB,'spectral qe'); % Sony color filters
routerCF = sensorGet(sensor_router,'spectral qe');

% XWdata_rgb * T = XWdata_XYZ
% T_rgb = inv(sonyCF'*sonyCF) * (sonyCF'*XYZ);
T_rgb = sonyCF \ XYZ;

%{
ieFigure([],'wide'); tiledlayout(2,2);
nexttile;
XYZ_pred = sonyCF*T_rgb;
plot(XYZ(:),XYZ_pred(:),'.');
...
xyz_rgb_patches = rgb_patches*T_rgb;
xyz_router_patches = router_patches1*T_router/4;

ieFigure;
scatter3(xyz_rgb_patches(:,1), xyz_rgb_patches(:,2), xyz_rgb_patches(:,3), 12, 'r', '.');
hold on;
scatter3(xyz_router_patches(:,1), xyz_router_patches(:,2), xyz_router_patches(:,3), 12, 'b', '.');
