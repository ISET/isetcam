...
% pixel
umPerDeg = 300;  arcminPerDeg = 60;                     %SI units
arcminPerPixel = 0.113;                                 %arcmin per pixel
umPerPixel = arcminPerPixel*(1/arcminPerDeg)*umPerDeg;  %um per pixel

% The cone mosaics for the individual subjects are pulled out and plotted
% here
...
L = (coneType == 1); M = (coneType ==2); S = (coneType == 3);
plot(xy(L,1),xy(L,2),'ro',xy(M,1),xy(M,2),'go',xy(S,1),xy(S,2),'bo');
...
save(subject,'xy','coneType','arcminPerPixel','umPerPixel','psf','comment')
