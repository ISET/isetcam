...
for ii = 1:length(subj)
    load(subj{ii})
    [r,c] = size(psf{1});
    x = (1:c)*umPerPixel; x = x - mean(x);
    y = (1:r)*umPerPixel; y = y - mean(y);
    ...
    L = find(fgrid == 1); M = find(fgrid == 2); S = find(fgrid == 3);
    ...
    figure(ii); imagescRGB(tmp); title(sprintf('Subject %s',subj{ii}))
end
